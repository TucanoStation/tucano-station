salvage-system-announcement-source = Sistema de Controle de Salvagem
salvage-system-announcement-arrived = Um pedaço de destroços foi puxado. Tempo estimado para soltar: { $timeLeft } segundos.
salvage-system-announcement-losing = O imã não consegue mais segurar os destroços. Tempo estimado até a perda: { $timeLeft } segundos.
salvage-system-announcement-lost = Os destroços foram perdidos.
salvage-system-announcement-spawn-magnet-lost = O imã de salvagem foi perdido.
salvage-system-announcement-spawn-no-debris-available = Nenhum destroço pôde ser recuperado pelo imã.
salvage-system-announcement-spawn-debris-disintegrated = Destroços desintegraram durante a transferência de órbita.
salvage-system-report-already-active = O imã já está ativo.
salvage-system-report-cooling-down = O imã está resfriando.
salvage-system-report-activate-success = O imã está puxando um pedaço de destroço!
salvage-system-magnet-examined-inactive = O imã está inativo.
salvage-system-magnet-examined-pulling-in = O imã está tentando puxar destroços.
salvage-system-magnet-examined-active =
    O imã está segurando um destroço. Consegue segurar por { $timeLeft ->
        [1] um segundo.
       *[other] { $timeLeft } segundos.
    }
salvage-system-magnet-examined-releasing = O imã está soltando o destroço.
salvage-system-magnet-examined-cooling-down = O imã está resfriando. Estará pronto em: { $timeLeft } segundos.
salvage-system-magnet-delay-upgrade = Atraso de acomplamento/cooldown
